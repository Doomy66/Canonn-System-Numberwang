...
def Misson_Gen(argv=''):
    ...
    for i, key in enumerate(faction_systems):
        ...
        if sys:
            ...
            if len(factions) > 1:
                gap = round(factions[0]["influence"] -
                            factions[1]["influence"], 1)
                gapfromtop = round(
                    factions[0]["influence"]-(empire['influence']), 1)
            else:
                gap = 100
                gapfromtop = 0
            ...
            system_overrides = list(
                filter(lambda x: x[0] == sys["system_name"], all_overrides))
                    ...
                    for f in faction_systems[key]['factions']:
                        if f['name'] in newmessage[2] and f['name'] != empire['name']:
                            ...
                            if len(f['conflicts']):  # There is a conflict
                                opponent = list(filter(
                                    lambda x: x['name'] == f['conflicts'][0]['opponent_name'], faction_systems[key]['factions']))
                                if opponent:
            ...
            if faction_systems[key]['override'] != 'Override':
                ...
                # DCOH Threat
                dcohthreat = next(
                    (x for x in dcoh if x['sys_name'] == sys["system_name"]), None)
            ...
            if len(sys['factions']) > 6:
                for faction in sys['factions']:
                    if next((x for x in faction['active_states'] if x['state'] == 'retreat'), None) or next((x for x in faction['pending_states'] if x['state'] == 'retreat'), None):
                        ...
                        messages.append(amessage(
                            sys, 7, f"Support {faction['name']} to be above 5% to prevent Retreat ({round(faction['influence'],1)}%)", dIcons['override']))
    ...
    for newmessage in all_overrides[1:]:
        if sum(faction_systems[x]["system_name"] == newmessage[0] for x in faction_systems) == 0:
            ...
            if exsys:
                ...
                message_gap = round(
                    message_inf - exsys['factions'][1]['influence'], 1) if len(exsys['factions']) > 1 else 0
                message_conflict = ''
                ...
                for f in exsys['factions']:
                    if f['name'] in newmessage[2]:
                        message_inf = round(f['influence'], 1)
                        message_gap = round(
                            exsys['factions'][0]['influence']-message_inf, 1)

                        if len(f['conflicts']):  # There is a conflict
                            opponent = list(filter(
                                lambda x: x['name'] == f['conflicts'][0]['opponent_name'], exsys['factions']))
                            if opponent:
                                message_conflict = f"({f['conflicts'][0]['status'].capitalize() if f['conflicts'][0]['status'] else 'Complete'} {f['conflicts'][0]['days_won']} v {opponent[0]['conflicts'][0]['days_won']})"
                                # print(message_conflict)

                newmessage[2] = newmessage[2].replace(
                    '{inf}', f"{message_inf}")
                newmessage[2] = newmessage[2].replace(
                    '{gap}', f"{message_gap}")
                newmessage[2] = newmessage[2].replace(
                    '{conflict}', f"{message_conflict}")
    ...
    for sys in invaders:
        ...
        # Policy is we allow NPC to arrive so they fill the system and block PC factions
        if sys['controlling_minor_faction'] in sys['pf']:
            messages.append(amessage(
                sys, 10, f"{sys['controlling_minor_faction']} ({round(sys['influence'],1)}%) will {sys['invademessage']} {'to' if sys['invadetype'][0]=='E' else 'in'} {sys['invading']} within {sys['cycles']} cycles : {('Support Non-Native Factions in '+sys['invading']) if sys['invadetype'][0]=='I' else ''}", dIcons['data']))
            # print('')
    ...
    for carrier in carriers:
        ...
        if carrier['id'][0] != '!':
            try:
                ...
                messages.append(amessage(
                    currentsystem, 9, f'{carrier["name"]} ({carrier["role"]})', dIcons['FC'], 'Canonn'))
            except:
                pass
    ...
    for x in oldmessage:
        s = list(filter(lambda y: y[1] == x[1] and y[8] not in [
    ...
    if CSNSettings.wh_id and len(list(filter(lambda x: x[0] < 11 or x[0] > 20, messagechanges))) > 0:
        ...
        print(
            f"Web Hook Text length is limited to 2000 chars : {len(wh_text)} + {len(wh_text_continued)}")
        CSNLog.info(
            f"Web Hook Text length is limited to 2000 chars : {len(wh_text)} + {len(wh_text_continued)}")
    ...
    print(
        '*** Missions Generated : Consuming {0} requests ***'.format(api.NREQ))
    if ('/wait') in argv:
...
def availableactions(system, factionnames):
    ...
    if 'commodities' in slist:
        actions += ['Trade']
    if 'exploration' in slist:
        actions += ['Data']
...
if __name__ == '__main__':
    Misson_Gen(sys.argv[1:] + ["/!schedule",
               "/!expansion", "/!new", "/!invade"])
    CSNLog.info('<<< End')
