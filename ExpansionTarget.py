...
def ExpansionToSystem(system, show=True, simpleonly=False, assumeretreat=False, easyinvade=False, live=False, avoidsystem=''):
    ...
    sysInRange = list(filter(
        lambda x: x['controllingFaction']['name'] not in factionspresent, sysInRange))

    print(f'# Looking for {"Simple " if simpleonly else""}expansions TO {system} in {len(sysInRange)} targets' + (
        ", assuming a Retreat happens" if assumeretreat else "") + (", avoiding "+avoidsystem if avoidsystem else ""))
    for i, sys in enumerate(sysInRange):
        update_progress(i/len(sysInRange), sys['name'])
        if sys['name'] == 'DEBUG':
            print('Debug')
        targets = ExpansionFromSystem(
            sys['name'], avoid_additional=avoidsystem)
        cycles = 0
    ...
    if show:
        print('')
        print(
            f"# Quickest Expansions to {system} which has {len(targetsys['factions'])} factions")
...
def ExpansionFromSystem(system_name, show=False, avoided_systems=None, avoid_additional=None, useretreat=True, asfaction=None, organisedinvasions=False, live=False, reportsize=8, extendedphase=False, simpleonly=False):
    ...
    if not avoided_systems:
        avoided_systems = list(x['name'] for x in spansh.systemspresent(
            asfaction if asfaction else sys['controllingFaction']['name']))
    ...
    sysInRange = spansh.cubearea(
        sys['name'], rangeSimple if simpleonly else rangeExtended, live=live)
    # Remove systems where faction is already present or other reasons
    sysInRange = list(filter(
        lambda x: x['name'] not in avoided_systems and x['name'] != sys['name'] and x['factions'], sysInRange))
    ...
    if len(sysInRange):
        ...
        for target in sysInRange:
            ...
            # Load Station and Beststation into System
            spansh.getstations(target['name'])
            ...
            # Simple Expansion
            if (target['cubedist'] <= rangeSimple or extendedphase) and numberoffactions < 7:
                target['sys_priority'] = sysdist(target, sys)
                target['expansionType'] = f"Expansion"
                target['expansionDetail'] = f"Simple"

                # Has previously retreated
                if useretreat and 'historic' in target.keys() and (asfaction if asfaction else sys['controllingFaction']['name']) in target['historic']:
                ...
                try:
                    ...
                    # Cannot Invade Controlling Faction
                    for targetfaction in target['factions'][:-1]:
                        ...
                        if targetfaction['name'] not in natives:
                            target['sys_priority'] = 200 + \
                                100*targetfaction['influence']
                            if organisedinvasions:
                                target[
                                    'expansionType'] = f"Invasion of {targetfaction['name']} (Organised)"
                                target['expansionDetail'] = f"{targetfaction['name']}"
                            else:
                                target[
                                    'expansionType'] = f"Invasion of {targetfaction['name']} ({round(100*targetfaction['influence'],1)}%)"
                                target['expansionDetail'] = f"{targetfaction['name']} ({round(100*targetfaction['influence'],1)}%)"
                            break
                except:
                    print(
                        f"!! Dodgy Faction {target['name']=} in {sys['name']=}")
        ...
        for cyclesys in sysInRange:
            # Simple (0+) Retreated (100+) and Invasion (200+) are 1 cycle
            cycles += 1 if cyclesys['sys_priority'] < 300 else 2
            cyclesys['cycles'] = cycles
        sysInRange = list(
            filter(lambda x: x['sys_priority'] != 1000, sysInRange))
...
def ExpansionCandidates(faction, show=False, prebooked=None, inflevel=CSNSettings.invasionparanoialevel, live=False, extendedphase=False):
    ...
    candidates = list(
        filter(lambda x: x['influence'] > inflevel/100 and x['factions'], candidates))
    ...
    for counter, c in enumerate(candidates):
        ...
        alltargets = ExpansionFromSystem(
            c['name'], avoid_additional=prebooked, live=live, extendedphase=extendedphase)
        if alltargets:
    ...
    savelist = []
    # Strip down data to stop Circular Ref during dump
    for e in list(filter(lambda x: x['expansion'], candidates)):
        savelist.append({'name': e['name'], 'target': e['expansion']
                        ['name'], 'expansionType': e['expansion']['expansionType']})
...
def InvasionAlert(faction, mininf=CSNSettings.invasionparanoialevel, show=True, lookahead=3, live=False):
    ...
    homesystems = list((x['name']
                       for x in spansh.systemspresent(faction, live=live)))
    donesystems = list()
    ...
    for counter, home in enumerate(homesystems):
        ...
        for invader in invaders:
            ...
            if targets:
                ...
                alertsystems[-1]['invademessage'] = 'Expand' if targets[0]['expansionType'][0] == 'E' else 'Invade ' + \
                    targets[0]['expansionDetail']
...
def InvasionRoute(start_system_name, destination_system_name, maxcycles=100, faction=None):
    ...
    def spider():
        ...
        thisstep = {'from': currentsys['name'], 'expansionType': None, 'to': None,
                    'routedist': route[-1]['routedist'], 'owner': None, 'sys': None}
        controlled = list()
        if faction:
            controlled = list(x['name']
                              for x in spansh.systemspresent(faction))
        ...
        if currentsys['name'] == sys_destination['name']:  # reached the end
            ...
            if (not bestdist) or route[-1]['routedist'] < bestdist:
                ...
                bestdist = min(
                    route[-1]['routedist'], bestdist) if bestdist else route[-1]['routedist']
        else:  # take the next set of expansions
            exp = ExpansionFromSystem(currentsys['name'], avoided_systems=controlled + list(
                x['to'] for x in route), useretreat=False, organisedinvasions=True)
            for sys in exp[:maxcycles]:
                ...
                # follow the expansion if it is usefull
                if xdist < currentdist and ((not bestdist) or thisstep['routedist'] < bestdist) and (sys['best'] > thisstep['routedist']):
                    print(
                        ' '*tdepth+f"{currentsys['name']} > {sys['name']} ({thisstep['expansionType']}) ({thisstep['routedist']}) ({round(xdist,1)})")
                    tdepth += 1
                    spider()
    ...
    print(
        f"Invasion Route from {start_system_name} to {destination_system_name}:")
    if not spansh:
        spansh = SpanshBGS()
    ...
    route = [{'from': start_system_name, 'expansionType': 'Start', 'to': start_system_name,
              'routedist': 0, 'owener': sys_start['controllingFaction']['name'], 'sys': sys_start}]
    allroutes = list()
    bestdist = None
    tdepth = 0
    spider()
    ...
    if allroutes:
        ...
        for n in allroutes[0][1:]:
            print(
                f"{n['from']} > {n['to']} ({n['owner']}): {n['expansionType']} ({n['routedist']})")
    else:
